...
local YBA = {
	...
	DropDownBG_BG_3 = Instance.new("ImageLabel"),
...
YBA.DropDownBG_BG_3.Name = "DropDownBG_BG_3"
YBA.DropDownBG_BG_3.Parent = YBA.DropDownBG_3
YBA.DropDownBG_BG_3.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
YBA.DropDownBG_BG_3.BackgroundTransparency = 1.000
YBA.DropDownBG_BG_3.Position = UDim2.new(-0.00999999978, 0, -0.00999999978, 0)
YBA.DropDownBG_BG_3.Size = UDim2.new(1.01999998, 0, 1.01999998, 0)
YBA.DropDownBG_BG_3.ZIndex = 0
YBA.DropDownBG_BG_3.Image = "rbxassetid://3570695787"
YBA.DropDownBG_BG_3.ImageColor3 = Color3.fromRGB(216, 34, 128)
YBA.DropDownBG_BG_3.ScaleType = Enum.ScaleType.Slice
YBA.DropDownBG_BG_3.SliceCenter = Rect.new(100, 100, 100, 100)
YBA.DropDownBG_BG_3.SliceScale = 0.120
...
BillboardGui.Name = "Custom_HealthBar"
...
local function pickupNearbyItems()
    for i, v in pairs(itemsFolder:GetChildren()) do
        if v.PrimaryPart ~= nil and toggles.ItemAutoFarm == true then
            ...
            --player.Character.HumanoidRootPart.CFrame = v.PrimaryPart.CFrame + Vector3.new(0, 0, 2)
            ...
            if clickDetector then
                ...
                TS:Create(player.Character.HumanoidRootPart, info, { CFrame = v.PrimaryPart.CFrame }):Play()
                wait(2)
    ...
    if itemInstance.PrimaryPart ~= nil then
        ...
        if searchForAllItems == true then
            ...
            if table.find(FilterItems, itemName) then
                ...
                if toggles.ItemAutoFarm == true then
                    isPickingUpItem = true
                    --player.Character.HumanoidRootPart.CFrame = itemInstance.PrimaryPart.CFrame + Vector3.new(0, 0, 2)
                    local clickDetector = itemInstance:FindFirstChild("ClickDetector")
                    if clickDetector then
                        local pos = player.Character.HumanoidRootPart.CFrame
                        local info = TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, false)
                        TS:Create(player.Character.HumanoidRootPart, info, { CFrame = itemInstance.PrimaryPart.CFrame }):Play()
                        wait(2)
                        --player.Character.HumanoidRootPart.Anchored = false
                        wait(1)
                        fireclickdetector(clickDetector, 1)
        ...
        for i, v in pairs(autoFarmPositionCFrames) do
            if toggles.ItemAutoFarm == true then
                if isPickingUpItem == false then
                    player.Character.HumanoidRootPart.CFrame = v + Vector3.new(0, 2, 0)
                end
            else
                break
            end
    ...
    if boolean == true then
        inputBegan_SuperSpeed = mouse.KeyDown:Connect(function(key)
            if key:lower()== superSpeed_KeyBind then
                ...
                velocity.velocity = (player.Character.Humanoid.MoveDirection) * superSpeed_speedValue
    ...
    while wait(.1) and toggles.playerHealthBars == true do
        for i, v in pairs(game:GetService("Players"):GetChildren()) do
            if v.Character and v ~= player then
                ...
                local healthBillboard = head:FindFirstChild("Custom_HealthBar")
                if healthBillboard then
                    local result = health.Value / health.MaxValue --new size
...
local function toggle_standAttach(playerName)
    ...
    if not standMorph then
        char.RemoteFunction:InvokeServer("ToggleStand", "Toggle")
        standMorph = char:FindFirstChild("StandMorph")
        standMorph:WaitForChild("HumanoidRootPart")
        standMorph.HumanoidRootPart.StandAttach:WaitForChild("AlignPosition")
        standMorph.HumanoidRootPart.StandAttach:WaitForChild("AlignOrientation")
    end
...
local function toggle_NPCFarm(npcName, amount)
    ...
    for _, v in pairs(game.Workspace.Living:GetChildren()) do
        if v.Name == npcName and v:FindFirstChildWhichIsA("Humanoid") then
            if v.Humanoid.Health > .2 then
                foundNPC = v
            end
        end
    end
    ...
    local isStuck = false
    local connection = RS.Heartbeat:Connect(function()
        if foundNPC and (toggles.NPCFarm == true or toggles.QuestFarm == true) and not isStuck then
...
local function toggle_QuestFarm(questTitle)
    ...
    elseif questTitle == "Dopey [Lvl. 30+]" then
        ...
        player.Character.RemoteEvent:FireServer("EndDialogue", {["NPC"] = "Dopey", ["Option"] = "Option1", ["Dialogue"] = "Dialogue3"}) --if first time
        toggle_NPCFarm("Deo", 1)
        
    elseif questTitle == "Deo [Lvl. 35+]" then
        player.Character.RemoteEvent:FireServer("EndDialogue", {["NPC"] = "Deo", ["Option"] = "Option1", ["Dialogue"] = "Dialogue4"}) --if first time
        toggle_NPCFarm("Joe", 1)
    end
...
game.StarterGui:SetCore("SendNotification",  {
    Title = "Credits";
    Text = "Made by zakater#0736";
    Icon = "rbxassetid://9309160657";
    Duration = 5;
})
